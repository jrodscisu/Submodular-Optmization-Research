...
% TODO: Add some background on combinatorial optimization, it could be that one paper that we have used before
...
We will also assume to have access to the submodular function by querying an \emph{oracle}. That is, we have access to the value of $f(S)$ for any $S \subseteq V$ through an \emph{oracle}. Besides we will also consider having a predictor that will provide a prediction in each step of the original greedy algorithm.

In section [A] we will go over our contributions, section [B] we will discuss our different type of predictions that we have considered, and section [C] we will present an example of a framework used to compare the performance of the different prediction scenarios. 
