...
int main(int argc, char * argv[]) {
    ...
    vector<priority_queue<pair<int, int>, vector<pair<int,int>>, greater<pair<int, int>> > > g;
    int cont = 0;
    int u, m, r;
    while(!fin.eof()){
        ...
        if(m_user.find(u) == m_user.end()){
            ...
            g.push_back(priority_queue<pair<int, int>, vector<pair<int, int> >, greater<pair<int, int> > > ());
        }
