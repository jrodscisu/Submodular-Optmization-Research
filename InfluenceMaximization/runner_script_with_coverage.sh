...
for i in $(seq 2 "$K"); do
    ...
    cargo run --bin tiptop "../data/bin/${GRAPH}.bin" "IC" "$i" "0.02" --threads 12 "$@"
    TIKTOP_EXIT_CODE=$?
    ...
    echo "Running Greedy Coverage graph=$GRAPH, k=$i"
    echo "Run $i of Greedy"
    ./../Coverage/src/greedy_rec "$i" "../Coverage/data/ML/ml-1m/${GRAPH}.txt" "data/temp_opt.txt"
