...
for dir in ../Coverage/data/ML/ml-1m/bygenre/*/; do
    if [ -d "$dir" ]; then
        for file in "$dir"*; do
            if [ -f "$file" ]; then
                # Remove the base path to get relative path
                relative_file=${file#../Coverage/data/ML/ml-1m/bygenre/}
                ...
                for i in $(seq "$minK" "$maxK"); do
                    ...
                    timeout $TIMEOUT cargo run --bin tiptop "../data/bin/bygenre/${relative_file%.txt}.bin" "IC" "$i" "0.02" --threads 12
                    TIKTOP_EXIT_CODE=$?
                    ...
                    ./../Coverage/src/greedy_bygenre "$i" "$file" "data/temp_opt.txt" "bygenre_output/${relative_file%.txt}"
